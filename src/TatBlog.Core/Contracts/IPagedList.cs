...
public interface IPagedList {
    // Tồng số trang (số tập con)
    int PageCount { get; }

    // Tổng số phần tử trả về từ truy vấn
    int TotalItemCount { get; }

    // Chỉ số trang hiện tại (bắt đầu từ số 0)
    int PageIndex { get; }

    // Vị trí của trang hiện tại (bắt đầu từ số 1)
    int PageNumber { get; }
    ...
    // Kiếm tra có trang kế tiếp hay không
...
public interface IPagedList<out T> : IPagedList, IEnumerable<T> {
    ...
    // Đếm số lượng phần tử chưa trong trang
    int Count { get; }
}
